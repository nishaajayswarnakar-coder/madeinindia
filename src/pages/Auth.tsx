  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'login') {
        ...
        return; // Skip turning off loading to preserve UI during redirect
      } else if (mode === 'forgot') {
      ...
      <div className="bg-white p-8 rounded-xl shadow-lg border border-slate-200">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-slate-500 mt-2">
            {mode === 'login' ? 'Sign in to access your dashboard' : mode === 'register' ? 'Join the B2B Industrial Hub' : 'Enter your email to receive a reset link'}
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {mode === 'register' && (
            <>
              {/* Other inputs will go here if needed, but we removed company and city */}
            </>
          )}
